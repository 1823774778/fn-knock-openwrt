...
const WXPUSHER_TARGET_SCHEMA: NotificationSchemaField[] = [
  ...
  },
  {
    ...
    label: "Topic",
    description:
      "必填。群发目标，可填写一个或多个 Topic ID，使用英文逗号或换行分隔。",
    placeholder: "123,456",
    type: "string",
    required: true,
  },
...
export const wxpusherProviderDefinition: NotificationProviderDefinition = {
  type: "wxpusher",
  label: "WxPusher",
  description: "通过 WxPusher 标准推送接口向指定 UID 或 Topic 发送消息通知。",
...
export const sendWxPusherMessage = async (args: {
  ...
  const uids = splitCommaSeparatedValues(targetConfig.uids);
  const { topicIds, invalidValues } = parseTopicIds(
    resolveFirstConfiguredValue(targetConfig, [
      "topic_ids",
      "topicIds",
      "topic_id",
      "topicId",
  ...
  if (uids.length === 0 && topicIds.length === 0) {
    return {
      success: false,
      retryable: false,
      reason: "WxPusher 至少需要配置一个 UID 或 Topic ID",
    };
  }

  const url =
    toTrimmedString(targetConfig.url) || resolvePrimaryActionUrl(args.message);
